...
export default class CheckoutController {
  public static async createOrder(ctx: Context) {
    if (ctx.session.orderId) {
      const order = await KlarnaService.getOrder(ctx.session.orderId)
    ...
    if (ctx.body !== null) {
      try {
        // Validate input
        const validatedOrder = await newOrderSchema.validate(ctx.body, {
          stripUnknown: true,
        })
        ...
        ctx.session.orderId = response.orderId
        ctx.body = response
      } catch (error) {
        if (has('errors', error)) {
          ctx.status = 400
  ...
  public static async getOrder(ctx: Context) {
    if (!ctx.params.id) {
      ctx.status = 500
  ...
  public static async confirmPurchase(ctx: Context) {
    if (!ctx.params.id) {
      ctx.status = 400
      ctx.body = 'You must specify an id as a parameter'
      ctx.app.emit('error', 'No id parameter was specified', ctx)
  ...
  public static async confirmKlarnaPush(ctx: Context) {
    if (!ctx.params.id) {
      ctx.status = 400
  ...
  public static async updateProducts(order) {
