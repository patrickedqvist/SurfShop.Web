...
const SizeAttribute = ({ label, onClick, selected }: SizeAttributeProps) => {
    ...
    return (
        <button className={classes} onClick={handleOnClick}>{label}</button>
    )
}
    ...
    return (
        <div className={'productAttributes'}>
            ...
            <div className={'productAttributes-options'}>
                {map((option) => <SizeAttribute key={option} label={option} onClick={onChange} selected={option === selectedOption} />, sortedAttributes)}
            </div>
        </div>
    )
}